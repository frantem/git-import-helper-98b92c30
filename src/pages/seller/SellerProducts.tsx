    ...
    try {
      ...
      if (editingProduct) {
        ...
        await Promise.all([
          ...
          selectedCategoryIds.length > 0 ? supabase.from("product_categories").insert(selectedCategoryIds.map(catId => ({ product_id: pid, category_id: catId }))) : Promise.resolve(),
    ...
    <div className="min-h-screen bg-background pb-16 md:pb-0">
      ...
      <main className="container mx-auto px-3 py-4">
        ...
        <div className="space-y-4">
          ...
          {products.length === 0 ? (
            <div className="py-8 text-center text-muted-foreground">У вас пока нет товаров</div>
          ) : (
        ...
        <div className="fixed inset-0 z-50 bg-background overflow-y-auto">
          ...
          <div className="p-4 pb-28 space-y-4">
            ...
            <div className="space-y-3 p-3 rounded-lg border border-border">
              ...
              <div className="flex items-center gap-2 flex-wrap">
                <Input value={mainVariant.label} onChange={(e) => setMainVariant({ ...mainVariant, label: e.target.value })} <Input value={mainVariant.label} onChange={(e) => setMainVariant({ ...mainVariant, label: e.target.value })} placeholder="250гр/4шт" className="h-9 w-24" /> className="h-9 w-24" />
                <div className="flex items-center gap-1">
          ...
          <div className="fixed bottom-0 left-0 right-0 p-4 bg-background border-t border-border z-20">
            <Button onClick={handleSaveProduct} disabled={isSaving} className="w-full h-12 text-base font-semibold">
              {isSaving ? "Сохранение..." : editingProduct ? "Сохранить изменения" : "Создать товар"}
            </Button>
          </div>
        </div>
      )}
